...
class Cia(
    ...
    /** Set by diagnostics to see which registers a program looks at, and what it got. Normally null. */
    var onRead: ((Int) -> Unit)? = null
    var onReadValue: ((Int, Int) -> Unit)? = null

    fun read(register: Int): Int = when (register and 0x0F) {
        0x0 -> {
            onRead?.invoke(0x0)
            ports.readPortA(this).also { onReadValue?.invoke(0x0, it) }
        }
        0x1 -> {
            onRead?.invoke(0x1)
            ports.readPortB(this).also { onReadValue?.invoke(0x1, it) }
        }
        0x2 -> portADirection
        0x3 -> portBDirection
        0x4 -> timerA and 0xFF
