...
class Drive1541(
    ...
    private val via1Ports = object : ViaPorts {
        override fun readPortB(via: Via6522): Int {
            // The lines read back inverted: a line held low reads as a one.
            var value = 0
            if (bus.data) value = value or 0x01
            if (bus.clock) value = value or 0x04
            if (bus.atn) value = value or 0x80
    ...
    private fun updateSerialOutputs(portB: Int) {
        val dataOut = portB and 0x02 != 0
        val clockOut = portB and 0x08 != 0
        ...
        // CA1 sees ATN through the same inverter as port B bit 7, so it follows it.
        via1.setCa1(bus.atn)
    }
